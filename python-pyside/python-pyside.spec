...
%{?filter_provides_in: %filter_provides_in %{python3_sitearch}/PySide/.*\.so}
%{?filter_setup}
...
mkdir -p python3
cd python3
%{cmake} -DCMAKE_SKIP_RPATH=true -DCMAKE_BUILD_TYPE=Release -DPYTHON_SUFFIX=.cpython-33m ../..
cd ..
popd

make -j16 %{?_smp_mflags} -C %{_target_platform}
make -j16 %{?_smp_mflags} -C %{_target_platform}/python3

%install
make -j4 install DESTDIR=$RPM_BUILD_ROOT -C %{_target_platform}
# Fix permissions
chmod 755 $RPM_BUILD_ROOT%{python_sitearch}/PySide/*.so

make -j4 install DESTDIR=$RPM_BUILD_ROOT -C %{_target_platform}/python3
# Fix permissions
chmod 755 $RPM_BUILD_ROOT%{python3_sitearch}/PySide/*.so
